...
import { xOn } from '../directives/compilable/xOn';
    ...
    if (segments) {
        ...
        for (const segment of segments) {
            if (isStatic) {
                ...
                const directiveData = xOn.middleware(segment, "", []) as any

                hydrationString += `
                handleDirective("x-text",
                document.querySelector("[data-hydration-id='${hydrationId}']"),
